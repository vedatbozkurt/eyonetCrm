...
@section('title', trans('backend.currencylist'))



@section('contentTitle', trans('backend.currencylist'))
...
@endif
<a href="{{ URL::to('crm/currency/create') }}" class="btn btn-success pull-right"><i class="fa fa-plus"></i>{{ __('backend.addnew') }}</a>
<table class="table table-hover table-bordered" id="sampleTable">
  <thead>
    <tr>
      <th>{{ __('backend.name') }}</th>
      <th>{{ __('backend.symbol') }}</th>
  ...
  <tbody>
   ...
   <tr>
    ...
    <td width="40%">{{ $row->symbol }}</td>
    <td width="10%">

      <a href="{{ route('currency.edit', $row->id) }}" class="btn btn-warning btn-sm"><i class="fa fa-edit"></i>{{ __('backend.edit') }}</a> </td>
      <td width="10%">
