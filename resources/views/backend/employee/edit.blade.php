...
@section('title', trans('backend.editemployee'))
@section('contentTitle', trans('backend.editemployee'))
@section('content')
{{-- {{ $data->description }} --}}
@if ($errors->any())
...
<form class="form-horizontal" enctype="multipart/form-data" method="post" action="{{ route('employee.update', $data->id) }}">
     @csrf
     @method('PATCH')
    <div class="tile-body">
...
@endif
        ...
        <div class="form-group row">
            ...
            <div class="col-md-8">
                <input class="form-control" type="password" name="password" placeholder="{{ __('backend.enterpassword') }}" required>
            </div>
        </div>
        <div class="form-group row">
